...
class DrydownModel:
    ...
    args = []
    mtype = None
    # TODO: Move type to here + rename to mtype. Probably need to fix in other files,
    ...
    def fit_event(self, event):
        ...
        try:
            ...
            event.add_results(self.mtype, results)
    ...
    def theta_w(self, event):
        if not self.specs['theta_w']: #and not self.mtype == 'exponential':
            return event.theta_w
        else:
            return np.array([
    ...
    def delta_theta(self, event=None, theta_0=None, theta_w=None):
        if theta_0 is None:
...
class NonlinearModel(DrydownModel):
    ...
    def theta_star(self, event):
        if not self.specs['theta_star']:
            ...
            return np.array([
                #0.0, # min_theta_star
                self.data.min_sm, # min_theta_star
